...
open class XAxisRenderer: AxisRendererBase
{
    ...
    open func drawLabels(context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        let labelAttrs = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): xAxis.labelFont,
        ...
        for i in stride(from: 0, to: entries.count, by: 1)
        {
            ...
            if viewPortHandler.isInBoundsX(position.x)
            {
                ...
                drawLabel(context: context,
                          ...
                          x: position.x,
                          y: pos,
                          attributes: labelAttrs as! [NSAttributedStringKey : NSObject],
        ...
        if limitLine.drawLabelEnabled && label.characters.count > 0
        {
            ...
            if limitLine.labelPosition == .rightTop
            {
                ChartUtils.drawText(context: context,
                    ...
                    align: .left,
                    attributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): limitLine.valueFont, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): limitLine.valueTextColor])
            }
